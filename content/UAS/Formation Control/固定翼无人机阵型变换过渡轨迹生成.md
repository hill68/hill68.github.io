...
2. **速度向量**
   ...
   $$
   \dot p_i(t)
   = \underbrace{\dot r(t)}_{\substack{\text{骨架切向}\\=V\,(\cos\psi_{\rm ref},\sin\psi_{\rm ref})}}
     + \underbrace{\dot R_c(t)\,\Delta_i(t)}_{\substack{\text{骨架旋转}\\\text{对偏移的贡献}}}
     + \underbrace{R_c(t)\,\dot\Delta_i(t)}_{\substack{\text{偏移本身}\\\text{的变化}}}.
   $$
