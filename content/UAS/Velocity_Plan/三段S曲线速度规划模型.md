...
该文档描述了算法的数学模型、约束条件以及求解流程。文档中的符号和公式与代码中的计算逻辑相对应，详细说明了如何根据输入参数（如轨迹弧长 S、总飞行时间 T、进入速度 vIn、期望速度 vDes、最小/最大速度、最大加速度 aMax 和最大加加速度 jMax）计算出整个轨迹在仿真步长 dt 下的速度、加速度和加加速度曲线，同时在后段尽可能保持接近期望速度飞行，并通过二分查找调整 vDes 以匹配目标弧长。
